...
if ($executores_da_tarefa_para_stop == 1) {
    ...
    if (!mysqli_query($conexao_select, $atualiza_tarefa)) {$erro_finaliza++;}
    ...
    if (!mysqli_query($conexao_select, $atualiza_a_tarefa)) {$erro_finaliza++; }
...
if (!mysqli_query($conexao_select, $atualiza_o_funcionario)) {$erro_finaliza++;}
...
while ($aux_result = mysql_fetch_array($result_flag_atividades)) {
    $cont_tarefas ++;
}


echo "Tarefa finalizada com sucesso!" . $cont_tarefas. $executores_da_tarefa_para_stop;
if ($cont_tarefas <= 1) {
...
if ($erro_finaliza == 0) {
        mysqli_commit($conexao_select);echo "Tarefa finalizada com sucesso!" . $cont_tarefas;
    } else {
        mysqli_rollback($conexao_select);
        echo "Nao foi possivel finalizar a tarefa";
    }
