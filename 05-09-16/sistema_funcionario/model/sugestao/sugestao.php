...
/*
    ...
    function addSugestao(sugestao $obj) {
        ...
        $pdo_insert_sugestao->select("insert into sugestoes (status,categoria_sugestao,data_enviada,nome_sugestao,dono_da_sugestao,como_e_hoje,como_deve_ser) values('nao vista','aberta','$data_hj','$nome_da_sugestao','$dono_sugestao','$como_e_hoje','$como_deve_ser')");
        $pdo_insert_sugestao->desconectar();
    }
    
    function getSugestoesPendentes($id_funcionario){
        $dona_da_sugestao = $this->getNomeFuncionario($id_funcionario);
        $pdo_sugestao_pendente = new Conexao();
        $result_sugestoes = $pdo_sugestao_pendente->select("select sugestoes.nome_sugestao,sugestoes.data_enviada from sugestoes where sugestoes.dono_da_sugestao = '$dona_da_sugestao'");
        $pdo_sugestao_pendente->desconectar();
        return $result_sugestoes;
     }
     */
